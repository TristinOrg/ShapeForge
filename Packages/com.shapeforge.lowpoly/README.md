...
Use `ShapeForge > Generate` in the Unity Editor to preview the articulated Sentinel Robot and detailed Inventor Workbench presets with full Undo support. The animated variants add a lightweight Low Poly example that controls cached `IShapeTransformTarget` instances without defining a ShapeForge animation format; a future motion system remains responsible for clips, tracks, interpolation, and serialization.
