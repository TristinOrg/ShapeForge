...
def optimize_model(model: dict[str, Any], evaluate: Evaluate, maximum_evaluations: int = 100,
                   ...
                   maximum_parameters_per_stage: int = 64) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    evaluation = 1
    accepted   = 0
    for stage in sorted({parameter.stage for parameter in parameters}):
        ...
        for parameter in stage_parameters:
            ...
            for direction in (1, -1):
                if evaluation >= maximum_evaluations:
                    break
                value = min(parameter.maximum, max(parameter.minimum, origin + direction * parameter.step))
                if value == origin:
                    continue
                candidate = copy.deepcopy(best)
                _set(candidate, parameter.path, round(value, 6))
                candidate_score, candidate_details = evaluate(candidate, evaluation)
                record = {"evaluation": evaluation, "score": candidate_score,
                          "parameter": parameter.key, "direction": direction,
                          "value": value, "accepted": False, "details": candidate_details}
